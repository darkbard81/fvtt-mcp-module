...
export class WebSocketManager {
  ...
  connect(): void {
    ...
    try {
      ...
      const customName = game.settings.get(moduleId as ClientSettings.Namespace, "customName" as ClientSettings.KeyFor<any>) as string;
      if (customName) {
  ...
  private onOpen(_event: Event): void {
    ...
    const pingIntervalSeconds = game.settings.get(moduleId as ClientSettings.Namespace, SETTINGS.PING_INTERVAL as ClientSettings.KeyFor<any>) as number;
  ...
  private scheduleReconnect(): void {
    ...
    const maxAttempts = game.settings.get(moduleId as ClientSettings.Namespace, SETTINGS.RECONNECT_MAX_ATTEMPTS as ClientSettings.KeyFor<any>) as number;
    const baseDelay = game.settings.get(moduleId as ClientSettings.Namespace, SETTINGS.RECONNECT_BASE_DELAY as ClientSettings.KeyFor<any>) as number;

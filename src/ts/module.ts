...
Hooks.once("init", () => {
  ...
  for (let [name, data] of Object.entries(SETTINGS.GET_DEFAULT())) {
    game.settings.register(CONSTANTS.MODULE_ID, name as ClientSettings.KeyFor<any>, <any>data);
  }
...
Hooks.on("renderSettingsConfig", (_: SettingsConfig, html: JQuery | HTMLElement) => {
  ...
  if (apiKeyInput.length) {
    ...
    apiKeyInput.on("change", (event) => {
      ...
      game.settings.set(moduleId as ClientSettings.Namespace, "apiKey" as ClientSettings.KeyFor<any>, newValue).then(() => {
        new Dialog({
  ...
  if (customNameInput.length) {
    customNameInput.on("change", (event) => {
      const newValue = (event.target as HTMLInputElement).value;
      game.settings.set(moduleId as ClientSettings.Namespace, "customName" as ClientSettings.KeyFor<any>, newValue).then(() => {
        new Dialog({
          title: "Reload Required",
